        ...
        .end(function (err, res) {
          chai.request(server)
            ...
            .end((err, resp) => {
              ...
              expect(resp.body.message).to.eql('Missing field validation error');
              done();
            });
        });
    });
          ...
          .end(function (err, res) {
            chai.request(server)
              ...
              .end((err, res) => {
                ...
                expect(res.body.pontos[0].latitude).to.be.eql(novaRota.pontos[0].latitude)
                expect(res.body.pontos[0].longitude).to.be.eql(novaRota.pontos[0].longitude)
                done();
              })
          })
      })
        ...
        .end(function (err, res) {
          antigaRota.save((err, user) => {
            chai.request(server)
              ...
              .set('auth-token', res.headers['auth-token'])
              .send(rota1).end((err, res) => {

                expect(res).to.have.status(404);
                expect(res.body.message).to.be.eql('Rota not found with id idInvalidooooooooooo');
                done();
              })
          })
        })
